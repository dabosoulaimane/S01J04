...
def journalist_nombre(maill)
	puts "Combien y a t'il de journaliste dans ce array ?"
	puts maill.size
end

journalist_nombre(mail)


# Combien de handle
def handle_number(maill)
	numero = maill.grep(/[0-9]/).count
	puts "Combien de handle contiennent un numéro ?"
	puts numero
end
...
def aude_handle(maill)
downcase = maill.map(&:downcase)
...
def commence_maj(maill)
majuscule = maill.grep(/@[A-Z]/).count
...
def under_score(maill)
underscore = maill.to_s.count("_")
...
def position(maill)
	puts "La position dans l'array de la personne @epenser est au #{maill.index("@epenser")}"
end
